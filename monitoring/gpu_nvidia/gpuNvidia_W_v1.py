    ...
    with open("gpu_data.txt", "w", encoding="utf-8") as f:
        if gpus_data:
            ...
            for gpu in gpus_data:
                ...
                log_output(f, f"  Core Clock (MHz): {gpu['Core Clock (MHz)']}")
                log_output(f, f"  Memory Clock (MHz): {gpu['Memory Clock (MHz)']}")
                log_output(f, f"  GPU Utilization (%): {gpu['GPU Utilization (%)']}")
                log_output(f, f"  Memory Utilization (%): {gpu['Memory Utilization (%)']}")
                log_output(f, f"  Memory Used (MB): {gpu['Memory Used (MB)']}")
                log_output(f, f"  Fan Speed (%): {gpu['Fan Speed (%)']}")
                log_output(f, f"  Temperature (C): {gpu['Temperature (C)']}")
                log_output(f, "-" * 40)
        else:
