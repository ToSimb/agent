...
output.append(f"Файл подкачки - всего: {swap.total if swap != -1 else '-1'} байт")
output.append(f"Файл подкачки - использовано: {swap.used if swap != -1 else '-1'} байт")
output.append(f"Файл подкачки - свободно: {swap.free if swap != -1 else '-1'} байт")
output.append(f"Загрузка CPU: {cpu_load if cpu_load != -1 else '-1'}%")
output.append(f"Физические ядра: {physical_cores if physical_cores != -1 else '-1'}")
output.append(f"Логические ядра: {logical_cores if logical_cores != -1 else '-1'}")
output.append(f"Прерывания в системе: {cpu_stats.interrupts if cpu_stats != -1 else '-1'}")  # Добавлен вывод количества прерываний
...
with open("system_info2.txt", "w", encoding="utf-8") as file:
    file.write("\n".join(output))
